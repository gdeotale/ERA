...
def gradcame(net, img, targ, image_tensor, var, transparency):
  target_layers = [net.model.layer3_R[var]]
  ...
  visualization = show_cam_on_image(input_image_np, grayscale_cam, use_rgb=True, image_weight=transparency)
  return input_image_np, visualization

def visualize_gradcam(net, image_tensor, targ, pred, classes, var):
  plt.subplots_adjust(left=0.1,
                    bottom=0.1,
                    right=0.9,
                    top=0.9,
  ...
  for img in range(maxi):
    input_image_np,visualization=gradcame(net, img, targ, image_tensor, var)
