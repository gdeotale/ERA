...
class BilingualDataset (Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # We will only add <5>, and </s> only on the label
        dec_num_padding_tokens = self.seq_len - len(dec_input_tokens) - 1

        # Make sure the number of padding tokens is not negative. If It is, the sentence 1s too long
        if enc_num_padding_tokens < 0 or dec_num_padding_tokens < 0:
            print(self.seq_len, len(enc_input_tokens), len(dec_input_tokens))
