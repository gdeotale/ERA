...
class InputEmbeddings(nn.Module):
    ...
    def forward(self, x):
        # (batch, seq_ len)--> (batch; seq_len, d_model)
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        pe[:, 1::2] = torch.cos(position * div_term) # cos(position * (10000 ** (21 / d _model))
    ...
    def forward(self, x):
        x = x + (self.pe[:, :x.shape[1], :]).requires_grad_(False) # (batch, seg_len, d_model)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        # Make sure d _model is divisible by h
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        # (batch, h, seq_len, d_Je) ~->- (batch, h, seq_len, seq_len)
    ...
    def forward(self, q, k, v, mask):
        query = self.w_q(q) # (batch, seq_len, «_model) --> (batch, seq_len, d_model) 
        key = self.w_k(k) # (batch, seq_len, d_model) --> (batch, seq_len, d_model) 
        value = self.w_v(v) # (batch, seg_len, d_model) --> (batch, seq_ten, d_model)
        
        # (batch, seq_len, d_model) --> (batch, seq_den, h, h_k) --> (batch, h, seq_len, d_k)
        ...
        # Multiply by Wo
        # (batch, seq len, d_model) --> (batch, seq_ len, d_model)
        return self.w_o(x)
